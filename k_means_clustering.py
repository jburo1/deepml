...
def k_means_clustering(points: list[tuple[float, float]], k: int, initial_centroids: list[tuple[float, float]], max_iterations: int) -> list[tuple[float, float]]:
    ...
    for _ in range(max_iterations):
        centroid_point_labels = np.array(
            [find_closest_centroid(point, centroids) for point in points])
        new_centroids = []
        for i in range(k):
...
print(k_means_clustering(points=[(1, 2), (1, 4), (1, 0), (10, 2), (10, 4), (
    10, 0)], k=2, initial_centroids=[(1, 1), (10, 1)], max_iterations=10))
