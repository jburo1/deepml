...
def create_exponent_configurations(feature_combinations: list[tuple], num_features):
    exponent_list = [[feature_combo.count(i) for i in range(
        num_features)] for feature_combo in feature_combinations]
...
def polynomial_features(X, degree):
    ...
    feature_combinations = [combo for i in range(
        degree+1) for combo in combinations_with_replacement(range(num_features), i)]
    ...
    poly_features = np.prod(monomials[:, np.arange(
        num_features), exponent_configurations], axis=2)
